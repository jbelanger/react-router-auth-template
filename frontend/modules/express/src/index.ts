 ...
 * ```
 */
export async function createExpressApp(config: Partial<ExpressServerConfig> = {}): Promise<Express> {
   ...
   log.info(
      '    ✓ session middleware (%s) with TTL: %ds',
   ...
   app.all(
      '*',
      reactRouterRequestHandler(environment.mode, environment.cwd, environment.reactRouterEntryPoint, viteDevServer),
   );
