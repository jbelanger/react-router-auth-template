...
const DEFAULT_CONFIG: Partial<SessionConfig> = {
   ...
   ignoredPaths: ['/api/buildinfo', '/api/health', '/api/readyz', '/.well-known/jwks.json'],
};
 ...
 * ```
 */
export function createSessionMiddleware(isProduction: boolean, config: SessionConfig): RequestHandler {
   ...
   const sessionStore =
      finalConfig.storageType === 'redis' && finalConfig.redisClient
         ? createRedisStore({
