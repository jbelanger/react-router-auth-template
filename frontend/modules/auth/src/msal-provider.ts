...
export async function createMsalProvider(config: AuthConfig): Promise<AuthProvider> {
   ...
   return {
      ...
      exchangeCodeForTokens: async (
         ...
         if (error) {
            ...
            throw new AuthError(`Authentication failed: ${errorDescription || error}`, error);
         }
      ...
      refreshAccessToken: async (
         ...
         const account = homeAccountId
            ? accounts.find((acc) => acc.homeAccountId.includes(homeAccountId))
            : accounts[0];
         if (!account) {
   ...
   async function getMetadata(
      config: AuthConfig,
   ): Promise<{ cloudDiscoveryMetadata: string; authorityMetadata: string }> {
      ...
      return {
         ...
         authorityMetadata:
            typeof authorityMetadata === 'string' ? authorityMetadata : JSON.stringify(authorityMetadata),
      };
   }
}
