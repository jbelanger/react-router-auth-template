...
export async function ensureUserHasApiAccess(session: Session, request: Request): Promise<UserData> {
    ...
    if (!user.apiToken || (user.apiToken && user.apiTokenExpiresAt && user.apiTokenExpiresAt - Date.now() < 5 * 60 * 1000)) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Backend authentication failed: ${response.status} ${response.statusText} - ${JSON.stringify(errorData)}`);
        }
